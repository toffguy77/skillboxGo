...
func GetUserFromServer(cCtx *cli.Context) error {
	...
	resp, err := http.Get(fmt.Sprintf("http://%s:%s/users/%s", server, port, strconv.Itoa(id)))
	if err != nil {
...
func DeleteUserFromServer(cCtx *cli.Context) error {
	...
	req, err := http.NewRequest("DELETE", fmt.Sprintf("http://%s:%s/users/%s", server, port, strconv.Itoa(id)), nil)
	if err != nil {
...
func UpdateUserAtServer(cCtx *cli.Context) error {
	...
	req, err := http.NewRequest(http.MethodPut, fmt.Sprintf("http://%s:%s/users/%s", server, port, strconv.Itoa(id)), bytes.NewBuffer(jsonData))
	if err != nil {
		return errors.New(fmt.Sprintf("can't get response: %v", err))
	}
...
func MakeFriendsOnServer(cCtx *cli.Context) error {
	...
	resp, err := http.Post(fmt.Sprintf("http://%s:%s/users/make_friend", server, port), "application/json",
...
func GetFriendsFromServer(cCtx *cli.Context) error {
	...
	resp, err := http.Get(fmt.Sprintf("http://%s:%s/friends/%s", server, port, strconv.Itoa(id)))
	if err != nil {
		return errors.New(fmt.Sprintf("can't get user %d: %v", id, err))
	}
	printResponse(resp)
